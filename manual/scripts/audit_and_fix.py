...
def fix_broken_images(text: str, all_stems: set[str]) -> tuple[str, int]:
    """존재하지 않는 이미지 링크 ![[...]] 삭제"""
    count = [0]
    def replacer(m: re.Match) -> str:
        s = m.group(1).split("|")[0].strip()
        # stem 이름만 체크 (확장자 포함)
        # all_stems 는 .md 파일만 있으므로 이미지는 항상 broken 으로 처리
        count[0] += 1
        return ""
...
def run(active_dir: str,
    ...
    for fname in md_files:
        ...
        # FIX-4: 깨진 이미지
        new_body, n = fix_broken_images(new_body, all_stems)
