"""
index_YYYYMMDD.md 생성 스크립트
────────────────────────────────────────────────────────
기능:
  active_YYYYMMDD/ 폴더의 모든 마크다운 파일을 날짜 역순으로 정렬하여
  월별 그룹핑된 인덱스 문서(index_YYYYMMDD.md)를 생성한다.
  파일명의 날짜 스탬프는 active 폴더명에서 자동 추출한다.

사용법:
    python gen_index.py <vault_active_dir>

예시:
    python gen_index.py ./vault/active_20260304/
...
def run(active_dir: str):
    ...
    for fname in os.listdir(active_dir):
        if not fname.endswith(".md") or fname.startswith("index_"):
            continue
    ...
    # 날짜 스탬프를 폴더명에서 추출 (active_20260304 → 20260304)
    folder_name = os.path.basename(os.path.normpath(active_dir))
    import re as _re
    m = _re.search(r'(\d{8})', folder_name)
    stamp = m.group(1) if m else datetime.now().strftime("%Y%m%d")

    out_path = os.path.join(active_dir, f"index_{stamp}.md")
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))

    print(f"index_{stamp}.md 생성 완료: {len(entries)}개 항목 → {out_path}")
